...
# Publish
curl -X POST -u $BINTRAY_API_KEY https://api.bintray.com/content/urbanairship/unity/unity-plugin/${VERSION}/publish

# Needed or show in download list fails because bintray thinks the release is not published yet
sleep 30

# Show in download list
curl -X PUT -H "Content-Type: application/json" -u $BINTRAY_API_KEY --data '{"list_in_downloads":true}' https://api.bintray.com/file_metadata/urbanairship/unity/unity-plugin/${VERSION}/urbanairship-${VERSION}.unitypackage